...
from ldm.inference_base import  get_adapters, get_base_argument_parser, get_sd_models
...
def t2i_ada():
    ...
    with torch.inference_mode(), \
        ...
        for cond_idx, cond_name in enumerate(activated_conds):
            ...
            #p.set_trace()
            # style feature 是一个tensor, sketch feature 是一个list, 好像style 特征直接用的VIT
        adapter_features, append_to_context = get_adapter_feature(conds, adapters)
    return adapter_features, sd_model
        #p.set_trace()
