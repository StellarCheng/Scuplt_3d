...
import sys
import cv2
sys.path.append('/home/chencheng/code/tool/threestudio/threestudio/models/T2I_ada')
...
parser.add_argument(
    '--sd_ckpt',
    type=str,
    default='/home/chencheng/code/tool/threestudio/threestudio/models/T2I_ada/models/v1-5-pruned-emaonly.ckpt',
    #default='/mnt/ssd1/chencheng/threestudio/threestudio/models/T2I_ada/models/v2-1_512-ema-pruned.ckpt',
...
global_opt.config = '/home/chencheng/code/tool/threestudio/threestudio/models/T2I_ada/configs/stable-diffusion/sd-v1-inference.yaml'
for cond_name in supported_cond:
    setattr(global_opt, f'{cond_name}_adapter_ckpt', f'/home/chencheng/code/tool/threestudio/threestudio/models/T2I_ada/models/coadapter-{cond_name}-sd15v1.pth')
...
coadapter_fuser.load_state_dict(torch.load(f'/home/chencheng/code/tool/threestudio/threestudio/models/T2I_ada/models/coadapter-fuser-sd15v1.pth'))
...
def read_img(image_path):
 ...
 #t2i_coada(vd_prompt, nf_depth_img,tmp_style_img, time_step)
def t2i_coada(prompt, depth_img, style_image,time_step,x_t):
    opt.prompt = prompt
    opt.outdir='/mnt/ssd1/chencheng/threestudio/threestudio/models/T2I_ada/outputs/test-composable-adapters'
    opt.b1 = "Nothing"  # 设置 btn1 的默认值
    #opt.depth_im = read_img(depth_img)  # 设置 im1  "/mnt/ssd1/chencheng/pnp-diffusers/data/yellow_duck_toy"
    opt.depth_im = depth_img #在之前就已经转成unit格式了
    opt.depth_cond_weight = 1.15  # 设置 cond_weight 的默认值

    opt.b2 = "Image"  # 设置 btn1 的默认值
    #opt.style_im = read_img(style_image)  # 设置 im1 
    opt.style_im = read_img(style_image)
    opt.style_cond_weight = 1.0 # 1.0  # 设置 cond_weight 的默认值
    ...
    #inps = [[opt.b1, opt.im1, opt.cond_weight1],[opt.b2, opt.im2, opt.cond_weight2]]
        ...
        #output =diffusion_inference(opt, sd_model, sampler, adapter_features, append_to_context)
        output = forward_unet(opt, sd_model, sampler, adapter_features, append_to_context,time_step=time_step,x_t=x_t)
        #base_count = len(os.listdir(opt.outdir))
        #cv2.imwrite(os.path.join(opt.outdir, f'{base_count:05}_result.png'), tensor2img(output))
        #cv2.imwrite(os.path.join(opt.outdir, f'{base_count:05}_result.png'), depth_img)
        #p.set_trace()
