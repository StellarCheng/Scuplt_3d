...
class PromptProcessorOutput:
    ...
    def get_text_embeddings(
        ...
        if view_dependent_prompting:
            # Get direction                direction_idx[d.condition(elevation, azimuth, camera_distances)]
            #p.set_trace()
            direction_idx = torch.zeros_like(elevation, dtype=torch.long)
            for d in self.directions:
                direction_idx[
                    d.condition(elevation, azimuth, camera_distances)
                ] = self.direction2idx[d.name]       #通过某个view为TRUE来取某个view
            #p.set_trace()
            # Get text embeddings
            ...
            ##这里会直接对齐view dependent prompt，不用更多操作###
            #return torch.cat([text_embeddings, uncond_text_embeddings], dim=0)
                        #text_embeddings: 1, 77, 1024
            #p.set_trace()
            return torch.cat([text_embeddings, uncond_text_embeddings], dim=0), direction_idx, self.prompt
        else:
...
class PromptProcessor(BaseObject):
    ...
    class Config(BaseObject.Config):
        ...
        front_threshold: float = 45.0   # config 里改到30
        back_threshold: float = 45.0    # config 里改到30
    ...
    def configure(self) -> None:
        ...
        if self.cfg.view_dependent_prompt_front:
            ...
            #这里normal到了-180到180,0-180不变，180到360对应负的角度也就是逆时针方向，所以定义顺时针为左，逆时针为右即可
            self.directions = [
                # DirectionConfig(
                #     "side",
                ...
                DirectionConfig(
                    "left",
                    lambda s: f"{s}, left view",
                    lambda s: s,
                    lambda ele, azi, dis: (
                        shift_azimuth_deg(azi) > 180 -self.cfg.front_threshold   #30
                    )
                    & (shift_azimuth_deg(azi) > self.cfg.front_threshold),
                ),
                DirectionConfig(
                    "right",
                    lambda s: f"{s}, right view",
                    lambda s: s,
                    lambda ele, azi, dis: (
                        shift_azimuth_deg(azi) > -180 +self.cfg.front_threshold   #30
                    )
                ...
                DirectionConfig(
                    ...
                    lambda s: s,
                    lambda ele, azi, dis: (
                        shift_azimuth_deg(azi) > -self.cfg.front_threshold   #30
                    )
                    & (shift_azimuth_deg(azi) < self.cfg.front_threshold),
                ),
                DirectionConfig(
                    ...
                    lambda s: s,
                    lambda ele, azi, dis: (
                        shift_azimuth_deg(azi) > 180 - self.cfg.back_threshold  #30
                    )
                    | (shift_azimuth_deg(azi) < -180 + self.cfg.back_threshold),
                ),
                DirectionConfig(
                    ...
                    lambda s: s,
                    lambda ele, azi, dis: ele > self.cfg.overhead_threshold, #60
                ),
            ]
