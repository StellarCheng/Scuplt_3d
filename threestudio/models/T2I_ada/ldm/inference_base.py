...
#这里是自己改的，用来预测噪声而不用跑完全程
def forward_unet(opt, model, sampler, adapter_features, append_to_context=None,time_step=None, x_t=None):
    # get text embedding
    c = model.get_learned_conditioning([opt.prompt])
    # import pdb as p
    # p.set_trace()
    #opt.neg_prompt=' '
    if opt.scale != 1.0:
        uc = model.get_learned_conditioning([opt.neg_prompt])  #只要用了负文本这里就以其作为unconditional enmbedding 
    else:
        uc = None
    c, uc = fix_cond_shapes(model, c, uc)
    #p.set_trace()
