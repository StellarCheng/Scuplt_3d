...
class ProlificDreamer(BaseLift3DSystem):
    ...
    def configure(self) -> None:
        ...
        super().configure()
        # self.cfg.prompt_processor_type 'stable-diffusion-prompt-processor'
        #self.cfg.prompt_processor {'pretrained_model_name_or_path': 'stabilityai/stable-diffusion-2-1-base', 'prompt': 'debug', 'front_threshold': 30.0, 'back_threshold': 30.0}
    ...
    def training_step(self, batch, batch_idx):
        ...
        #loss=loss*0.05
        # loss=loss*0.1
        # loss += guidance_out['vit_ssim']+guidance_out['vit_entire_cls'] #手动设置成了1，这里
        # self.log("vit_ssim", guidance_out['vit_ssim'])
        # self.log("vit_entire_cls", guidance_out['vit_entire_cls'])
        ...
        if self.cfg.stage == "coarse":
            ...
            if (
                ...
                #p.set_trace()
                if 'shape_points' in out.keys():
                    loss_shape = self.shape_loss(out["shape_points"], out["shape_density"])
                else:
