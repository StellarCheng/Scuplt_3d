...
def ce_pq_loss(p, q, weight=None):
    ...
    p = p.view(q.shape)
    ce = -1 * (q * torch.log(clamp(p)) + (1 - q) * torch.log(clamp(1 - p)))
...
class ShapeLoss(nn.Module):
    def __init__(self, guide_shape):
        ...
        if self.shape_path.endswith('glb'):
            v,f =self.read_glb(self.shape_path)
        else:
            try:
                import re
                #'/home/chencheng/code/tool/threestudio/load/T3B_Single_obj/A_crisp_paper_airplane.obj'
                self.shape_path = re.sub(r'^.*/load/', './load/', self.shape_path)
                v, f = pcu.load_mesh_vf(self.shape_path)
                #v, _, _, f, _, _ = read_obj(self.shape_path, float)
            except:
                p.set_trace()
                mesh = trimesh.load(self.shape_path)
                #p.set_trace()
                v = mesh.vertices
                f = mesh.faces
        #v, _, _, f, _, _ = read_glb(self.shape_path, float)
        mesh = MeshOBJ(v, f)
    ...
    def read_glb(self, glb_path):
        scene = trimesh.load(glb_path)

        # Access the first mesh in the scene
        mesh = scene.geometry[list(scene.geometry.keys())[0]]

        # Get the vertices and faces data
