...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    def __init__(self, cfg: Any) -> None:
        ...
        # ]
        ##changed by cheng
        if isinstance(self.heights, int):
            self.directions_unit_focals = [
            get_ray_directions(H=height, W=width, focal=1.0)
            for (height, width) in zip(self.heights, self.widths)]
        else:
            self.directions_unit_focals = [
            get_ray_directions(H=height, W=width, focal=1.0)
            for (height, width) in zip(self.heights[0], self.widths[0])]
    ...
    def update_step(self, epoch: int, global_step: int, on_load_weights: bool = False):
        #update by chen using both 64 and 512 for shape and texture
        size_ind = bisect.bisect_right(self.resolution_milestones, global_step) - 1
        self.height = self.heights[size_ind]
        if self.height[1]<512:
            self.height: int = self.heights
            self.width: int = self.widths
            self.directions_unit_focal = self.directions_unit_focals
        p.set_trace()
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        if random.random() < 0.5:
            ...
            elevation_deg = (
                ...
                * (self.cfg.elevation_range[1] - self.cfg.elevation_range[0])
                + self.cfg.elevation_range[0]    #(-10, 90)
            )
        ...
        import pdb as p
        #p.set_trace()
        if isinstance(self.directions_unit_focal, list):
            rays_o, rays_d={}, {}

            focal_length: Float[Tensor, "B"] = 0.5 * self.height[1] / torch.tan(0.5 * fovy)

            directions: Float[Tensor, "B H W 3"] = self.directions_unit_focal[1][
                None, :, :, :
            ].repeat(self.cfg.batch_size, 1, 1, 1)
            directions[:, :, :, :2] = (
                directions[:, :, :, :2] / focal_length[:, None, None, None]
            )
            rays_o['texture'],  rays_d['texture'] = get_rays(directions, c2w, keepdim=True)

            proj_mtx: Float[Tensor, "B 4 4"] = get_projection_matrix(
                fovy, self.width[1] / self.height[1], 0.1, 1000.0
            )  # FIXME: hard-coded near and far
            ###shape added by cheng
            focal_length: Float[Tensor, "B"] = 0.5 * self.height[0] / torch.tan(0.5 * fovy)

            directions_shape: Float[Tensor, "B H W 3"] = self.directions_unit_focal[0][
                None, :, :, :
            ].repeat(self.cfg.batch_size, 1, 1, 1)
            directions_shape[:, :, :, :2] = (
                directions_shape[:, :, :, :2] / focal_length[:, None, None, None]
            )
            rays_o['shape'],  rays_d['shape'] = get_rays(directions_shape, c2w, keepdim=True)
            p.set_trace()
        else:
            focal_length: Float[Tensor, "B"] = 0.5 * self.height / torch.tan(0.5 * fovy)
            ...
            proj_mtx: Float[Tensor, "B 4 4"] = get_projection_matrix(
            fovy, self.width / self.height, 0.1, 1000.0 )  # FIXME: hard-coded near and far
        ...
        #p.set_trace()
        return {
            "rays_o": rays_o,  # Size([1, 64, 64, 3])
            "rays_d": rays_d,  
            "mvp_mtx": mvp_mtx, # [1,4,4]
            "camera_positions": camera_positions, #[1,3]
            "c2w": c2w,      # Size([1, 4, 4])
            "light_positions": light_positions, #[1,3]
            "elevation": elevation_deg, #[1]
            "azimuth": azimuth_deg, #[1]
            "camera_distances": camera_distances, #[1]
            "height": self.height,
            "width": self.width,
        }
